...
<!--control-->
<br>
<a href="/stop"><button>Stop Server</button></a>
'''
...
def usage():
    print "ytdl_server.py [-p=port] [-f=formatstring] [-t=preset] [-s=style]"
...
def get_dirpage(path):
    ...
    if files or dirs:
        ...
        if files:
            dircontent += '<a href="/dplay?url=http://localhost:9192/playlist?path='+quote(path)+'&mode=o&name=pl.m3u">Files</a>'
        if dirs:
            dircontent += ' <a href="/dplay?url=http://localhost:9192/playlist?path='+quote(path)+'&mode=or&name=pl.m3u">Recursive</a>'
        dircontent += '</h6><h6>Shuffle: '
        if files:
            dircontent += '<a href="/dplay?url=http://localhost:9192/playlist?path='+quote(path)+'&mode=s&name=pl.m3u">Files</a>'
        if dirs:
            dircontent += ' <a href="/dplay?url=http://localhost:9192/playlist?path='+quote(path)+'&mode=sr&name=pl.m3u">Recursive</a>'
        dircontent += '</h6><br>'
    if playlists:
...
index_page = base_page.replace('$script$','').replace('$title$','Youtube-DL-Server').replace('$content$',index_content)
