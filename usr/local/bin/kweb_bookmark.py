...
class App(tk.Frame):              
    def __init__(self, master=None,uri='',name='',errmsg='',msg='',):
        ...
        self.name.set(name)
        self.newinstance = tk.IntVar()
        self.newinstance.set(0)
        self.uri = uri
        self.errmsg = errmsg
        self.msg = msg
        self.bmarr = []
    ...
    def createWidgets(self):
        if self.errmsg:
            ...
            self.ckeckinstance =tk.Checkbutton(self,text="Open in new browser instance",font=self.font,variable=self.newinstance)
            self.ckeckinstance.grid(row=2,column=0,columnspan=2, sticky=tk.E+tk.W)
            ...
            self.mtext2.grid(row=3,column=0,columnspan=2,sticky=tk.N+tk.S+tk.E+tk.W)
            ...
            self.yScroll.grid(row=4,column=2, sticky=tk.N+tk.S)
            self.bmlist.grid(row=4,column=0,columnspan=2, sticky=tk.N+tk.S+tk.E+tk.W)
            self.quitbutton=tk.Button(self, takefocus=0, command=self.quitapp, font=self.font, text = 'Cancel', justify = tk.CENTER)
            self.quitbutton.grid(row=5,column=0,sticky=tk.W)
            self.savebutton=tk.Button(self, takefocus=0, command=self.savebm, font=self.font, text = 'Save', justify = tk.CENTER)
            self.savebutton.grid(row=5,column=1,sticky=tk.E)
