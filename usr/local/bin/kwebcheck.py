...
def check_ytdl_pip():
    ret = False
    try:
        checkpip = subprocess.Popen(['pip','show','youtube-dl'],stdout=subprocess.PIPE,stderr=subprocess.PIPE)
...
if os.path.exists('/usr/bin/youtube-dl') or os.path.exists('/usr/local/bin/youtube-dl'):
    from_pip = check_ytdl_pip()
    ytdldir = os.path.expanduser('~')+os.sep+'youtube-dl'
    if os.path.exists(ytdldir):
        print "found: youtube-dl from github (best solution)"
    else:
        if from_pip:
            print "found: youtube-dl installed with pip. This will work."
            print "you can skip the installation from github later on."
        else:
